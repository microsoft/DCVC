...
void RansEncoderLib::encode_with_indexes(
    const std::vector<int16_t> &symbols, const std::vector<int16_t> &indexes,
  ...
  for (int i = 0; i < symbol_size; ++i) {
    ...
    const int32_t *cdf = cdfs[cdf_idx].data();
...
void RansDecoderLib::set_stream(const std::vector<uint8_t> &encoded) {
  _stream = encoded;
...
std::vector<int16_t>
RansDecoderLib::decode_stream(const std::vector<int16_t> &indexes,
                              const std::vector<std::vector<int32_t>> &cdfs,
                              const std::vector<int32_t> &cdfs_sizes,
                              const std::vector<int32_t> &offsets) {
...
std::vector<int16_t> RansDecoderLib::get_decoded_symbols() {
  return m_decodedSymbols;
}
