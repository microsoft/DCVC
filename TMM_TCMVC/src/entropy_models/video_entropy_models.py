...
class EntropyCoder(object):
    ...
    def set_decoder_cdf(self, cdf, cdf_length, offset):
        self.decoder.set_cdf(cdf, cdf_length, offset)
    ...
    def decode_with_indexes(self, strings, indexes, cdf, cdf_length, offset):
        values = self.decoder.decode_with_indexes(strings, indexes, cdf, cdf_length, offset)
        values = torch.Tensor(values).reshape(-1)
        return values.float()
    ...
    def decode_stream_only_indexes(self, indexes):
        rv = self.decoder.decode_stream_only_indexes(indexes)
        return rv
...
class BitEstimator(nn.Module):
    ...
    def encode(self, x):
        ...
        return self.entropy_coder.encode_with_indexes(x.reshape(-1).int().tolist(),
    ...
    def decode_stream(self, size):
        ...
        val = self.entropy_coder.decode_stream(indexes.reshape(-1).int().tolist(),
                                               *self.cdf_helper.get_cdf_info_list())
...
class GaussianEncoder(object):
    ...
    def set_decoder_cdf(self):
        self.entropy_coder.set_decoder_cdf(*self.cdf_helper.get_cdf_info_list())
    ...
    def decode_stream_only_indexes(self, indexes):
        return self.entropy_coder.decode_stream_only_indexes(indexes)
