...
void BufferedRansEncoder::encode_with_indexes(
  ...
  for (size_t i = 0; i < symbols.size(); ++i) {
    ...
    assert(cdf_idx >= 0);
    assert(cdf_idx < static_cast<int>(cdfs.size()));

    const auto &cdf = cdfs[cdf_idx];

    const int32_t max_value = cdfs_sizes[cdf_idx] - 2;
    assert(max_value >= 0);
    assert((max_value + 1) < static_cast<int>(cdf.size()));
...
RansDecoder::decode_with_indexes(const std::string &encoded,
  ...
  for (int i = 0; i < static_cast<int>(indexes.size()); ++i) {
    ...
    const auto it = std::find_if(cdf.begin(), cdf_end, [cum_freq](int v) {
      return static_cast<uint32_t>(v) > cum_freq;
    });
    assert(it != cdf_end + 1);
    const uint32_t s =
        static_cast<uint32_t>(std::distance(cdf.begin(), it) - 1);
...
void RansDecoder::set_cdf(const std::vector<std::vector<int32_t>> &cdfs,
                          const std::vector<int32_t> &cdfs_sizes,
                          const std::vector<int32_t> &offsets) {
  m_cdfs = cdfs;
  m_cdfs_sizes = cdfs_sizes;
  m_offsets = offsets;
}

std::vector<int32_t>
RansDecoder::decode_stream_only_indexes(const std::vector<int32_t> &indexes) {
  return decode_stream(indexes, m_cdfs, m_cdfs_sizes, m_offsets);
}
...
PYBIND11_MODULE(MLCodec_rans, m) {
  ...
  py::class_<RansDecoder>(m, "RansDecoder")
      ...
      .def("set_cdf", &RansDecoder::set_cdf)
      ...
      .def("decode_stream_only_indexes",
           &RansDecoder::decode_stream_only_indexes,
           "Decode a string to a list of symbols");
}
