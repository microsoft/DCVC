 ...
 *   1. cutlass::gemm::kernel::GemmUniversal -> QuadGemmUniversal
 */
...
template <class ProblemShape_, class CollectiveMainloop_, class CollectiveEpilogue_,
          class TileSchedulerTag_ = void, class Enable = void>
class QuadGemmUniversal;

// Original file: cutlass/gemm/kernel/sm90_gemm_tma_warpspecialized_cooperative.hpp
...
class QuadGemmUniversal<
    ProblemShape_, CollectiveMainloop_, CollectiveEpilogue_, TileSchedulerTag_,
    cute::enable_if_t<cute::is_base_of_v<cutlass::gemm::KernelTmaWarpSpecializedCooperative,
...
public:
    ...
    using TileScheduler = typename cutlass::gemm::kernel::detail::TileSchedulerSelector<
        TileSchedulerTag, ArchTag, TileShape, ClusterShape, TileSchedulerPipelineStageCount>::Scheduler;
    ...
    static constexpr uint32_t NumMMAThreads = cute::size(TiledMma{});               // 8 warps
    static constexpr uint32_t NumMainloopLoadThreads = cutlass::NumThreadsPerWarp;  // 1 warp
    static constexpr uint32_t NumEpilogueLoadThreads = cutlass::NumThreadsPerWarp;  // 1 warp for C
    ...
    static constexpr bool IsMainloopAuxiliaryLoadNeeded =
        cutlass::gemm::kernel::detail::HasAuxiliaryLoad_v<typename CollectiveMainloop::DispatchPolicy>;

    /// Register requirement for Load and Math WGs
    static constexpr int RegsPerThread = cute::size<0>(TileShape{}) * cute::size<1>(TileShape{})
                                         / NumMMAThreads * sizeof(ElementAccumulator)
                                         / sizeof(uint32_t);
    ...
    {
        bool implementable = (args.mode == cutlass::gemm::GemmUniversalMode::kGemm)
                             or (args.mode == cutlass::gemm::GemmUniversalMode::kBatched
                                                ...
                                                cutlass::CudaHostAdapter* cuda_adapter = nullptr)
    {
        ...
        status = CollectiveEpilogue::initialize_workspace(args.problem_shape, args.epilogue,
                                                          workspace_ptr + workspace_offset, stream,
                                                          cuda_adapter);
        ...
        status = TileScheduler::template initialize_workspace<ProblemShape, ElementAccumulator>(
            args.scheduler, workspace_ptr + workspace_offset, stream, args.problem_shape,
            args.hw_info, NumMmaWarpGroups, NumEpilogueSubTiles, NumAccumulatorMtxs, cuda_adapter);
    ...
    {
#ifdef __CUDA_ARCH_FEAT_SM90_ALL
    #define ENABLE_SM90_KERNEL_LEVEL 1
#endif
// Any Tensor Op MMA Atom in the ISA is arch conditional.
#ifndef ENABLE_SM90_KERNEL_LEVEL
        printf(
        ...
        // Preconditions
        static_assert(NumMMAThreads == 256,
        ...
        static_assert(cute::rank(StrideC{}) == 3,
                      "StrideC must be rank-3: [M, N, L]. If batch mode is not needed, set L "
                      "stride to Int<0>.");
        ...
        // (BLK_M,BLK_K,m,k,l) get<1>(load_inputs) is the tma tensor B0 after local tiling so that
        // it has shape (BLK_N,BLK_K,n,k,l) get<2>(load_inputs) is the tma tensor B1 after local
        // tiling so that it has shape (BLK_N,BLK_K,n,k,l) get<3>(load_inputs) is the tma tensor B2
        // after local tiling so that it has shape (BLK_N,BLK_K,n,k,l) get<4>(load_inputs) is the
        // tma tensor B3 after local tiling so that it has shape (BLK_N,BLK_K,n,k,l)

        auto load_inputs = collective_mainloop.load_init(problem_shape_MNKL, params.mainloop);
        static_assert(cute::tuple_size_v<decltype(load_inputs)> >= 5,
                      "Output of load_init must have at least five elements (A, B0, B1, B2, B3)");
        ...
        if (warp_group_role == WarpGroupRole::Producer) {
            ...
            if (producer_warp_role == ProducerWarpRole::Warp1) {
                ...
                if (producer_warp_role == ProducerWarpRole::Mainloop) {
                    ...
                    while (work_tile_info.is_valid()) {
                        ...
                        auto m_coord = cute::idx2crd(work_tile_info.M_idx, cute::shape<2>(gA_mkl));
                        auto n_coord = cute::idx2crd(work_tile_info.N_idx, cute::shape<2>(gB0_nkl));
                        auto l_coord = cute::idx2crd(work_tile_info.L_idx, cute::shape<4>(gB0_nkl));
                        auto blk_coord = cute::make_coord(m_coord, n_coord, cute::_, l_coord);
                        ...
                        auto k_tile_iter = cute::make_coord_iterator(
                            cute::idx2crd(work_k_tile_start, cute::shape<3>(gA_mkl)),
                            cute::shape<3>(gA_mkl));
                    ...
                    if constexpr (IsMainloopAuxiliaryLoadNeeded) {
                        while (work_tile_info.is_valid()) {
                            ...
                            auto m_coord = cute::idx2crd(work_tile_info.M_idx, cute::shape<2>(gA_mkl));
                            auto n_coord =
                                cute::idx2crd(work_tile_info.N_idx, cute::shape<2>(gB0_nkl));
                            auto l_coord =
                                cute::idx2crd(work_tile_info.L_idx, cute::shape<4>(gB0_nkl));
                            auto blk_coord = cute::make_coord(m_coord, n_coord, cute::_, l_coord);
                    ...
                    while (work_tile_info.is_valid()) {
                        if (TileScheduler::compute_epilogue(work_tile_info, params.scheduler)) {
                            ...
                            epi_load_pipe_producer_state = collective_epilogue.load(
                                ...
                                blk_shape, blk_coord, tiled_mma, lane_idx,
                                shared_storage.tensors.epilogue,
            ...
            while (work_tile_info.is_valid()) {
                ...
                auto accumulators0 =
                    cute::partition_fragment_C(tiled_mma, cute::take<0, 2>(blk_shape));  // (MMA,MMA_M,MMA_N)
                auto accumulators1 =
                    cute::partition_fragment_C(tiled_mma, cute::take<0, 2>(blk_shape));  // (MMA,MMA_M,MMA_N)
                auto accumulators2 =
                    cute::partition_fragment_C(tiled_mma, cute::take<0, 2>(blk_shape));  // (MMA,MMA_M,MMA_N)
                ...
                if (TileScheduler::valid_warpgroup_in_work_tile(work_tile_info)) {
                    collective_mainloop.mma(mainloop_pipeline, mainloop_pipe_consumer_state,
                                            ...
                                            accumulators3, work_k_tile_count, mma_thread_idx,
                ...
                TileScheduler::fixup(params.scheduler, work_tile_info, accumulators1,
                                     NumMmaWarpGroups, consumer_warp_group_idx);
                TileScheduler::fixup(params.scheduler, work_tile_info, accumulators2,
                                     NumMmaWarpGroups, consumer_warp_group_idx);
                TileScheduler::fixup(params.scheduler, work_tile_info, accumulators3,
                                     NumMmaWarpGroups, consumer_warp_group_idx);
                ...
                if (TileScheduler::compute_epilogue(work_tile_info, params.scheduler)) {
                    ...
                    auto [epi_load_pipe_consumer_state_next, epi_store_pipe_producer_state_next] =
                        collective_epilogue.store(
                            ...
                            accumulators0, accumulators1, accumulators2, accumulators3, tiled_mma,
                            mma_thread_idx, shared_storage.tensors.epilogue,
                            work_tile_info.reduction_subtile_idx());
    ...
    static Params to_underlying_arguments(Arguments const& args, void* workspace)
    {
        ...
        if (sm_count <= 0) {
            CUTLASS_TRACE_HOST(
                ...
                "  For optimal performance, populate the arguments KernelHardwareInfo struct with "
                "the SM count.");
            sm_count =
        ...
        if (max_active_clusters <= 0) {
            ...
            CUTLASS_TRACE_HOST(
                ...
                "  For optimal performance, populate the arguments KernelHardwareInfo struct with "
                "the max_active_clusters.");
        } else {
            CUTLASS_TRACE_HOST(
                "to_underlying_arguments(): Setting persistent grid cluster count to "
                << max_active_clusters);
        }

        cutlass::KernelHardwareInfo hw_info{ args.hw_info.device_id, sm_count, max_active_clusters };
